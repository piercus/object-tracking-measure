...
module.exports = function (options) {
	...
	*/
	...
	const mt = idtpSet.map(({indexes}) => groundTruths[vt[indexes[0]].gtIndex]);
	const mc = idtpSet.map(({indexes}) => predictions[vc[indexes[1]].predIndex]);

	const idfn = sum(idtpSet.map((_, i) => detailedDistFn(mt[i], mc[i]).fn));
	const idfp = sum(idtpSet.map((_, i) => detailedDistFn(mt[i], mc[i]).fp));

	const idtp1 = sum(mc.map(p => p.filter(a => a).length)) - idfn;
	const idtp2 = sum(mt.map(g => g.filter(a => a).length)) - idfp;
