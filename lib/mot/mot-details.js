...
module.exports = function (options) {
	...
	const t = groundTruths.map(a => a.filter(Boolean).length).reduce((a, b) => a + b, 0);
	...
	for (let frameIndex = 0; frameIndex < nFrames; frameIndex++) {
		...
		const phiTIndexes = currentCouples.filter(([gt, pred]) => ((typeof (newTrackerToGt[pred]) === 'number') && newTrackerToGt[pred] !== gt)
				|| (((newTrackerToGt[pred]) === undefined) && (groundTruths[gt][frameIndex] !== null && (groundTruths[gt][frameIndex]) !== undefined))).map(([gt, pred]) => ([pred, gt, newTrackerToGt[pred]]));
	...
	const result = {t, fp, fn, phi, c, dist, allMatched, fnIndexes, fpIndexes, allPhiTIndexes, nPrediction: predictions.length, nGroundTruth: groundTruths.length};
	if (logger) {
