...
module.exports = function (options) {
	...
	for (let frameIndex = 0; frameIndex < nFrames; frameIndex++) {
		...
		const phiTIndexes = currentCouples.filter(([gt, pred]) => newTrackerToGt[pred] !== gt).map(([gt, pred]) => ([pred, gt, newTrackerToGt[pred]]));
	...
	if (fn + fp + phi > t) {
		throw (new Error(`Invalid motDetails data (fn+fp+phi > t) fn: ${fn}, fp: ${fp}, phi: ${phi}, t: ${t}`));
	}
